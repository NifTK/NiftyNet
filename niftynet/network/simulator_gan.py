...
class ImageGenerator(BaseGenerator):
    ...
    def layer_op(self, random_source, image_size, conditioning, is_training):
        ...
        ch = [noise_size+conditioning_channels*generator_shortcuts[0]]
        sz = [image_size[:-1]]
        keep_prob_ph = 1  # not passed in as a placeholder
        for i in range(4):
            ch.append(ch[-1]//2+conditioning_channels*generator_shortcuts[i+1])
        ...
        def up_block(ch, x, i):
            with tf.name_scope('up_block'):
                return conv(ch, concat_cond(up(ch, x), i))
