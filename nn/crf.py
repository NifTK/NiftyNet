...
def CRFAsRNN(I,U,T=5,aspect_ratio=[1.,1.,1.], alpha=5.,beta=5.,gamma=5.,name='CRFAsRNN'):
	# As in https://arxiv.org/pdf/1210.5644.pdf we use a bilateral and a spatial kernel
	# which simplifies the whitening of the features to a elementwise multiplication
	# This could be made more general if needed
	batch_size=int(U.get_shape()[0])
	H1=[U]
	# Build permutohedral structures for smoothing
	coords=tf.tile(tf.expand_dims(tf.stack(tf.meshgrid(*[numpy.array(range(int(i)),dtype=numpy.float32)*a for i,a in zip(U.get_shape()[1:4],aspect_ratio)]),3),0),[batch_size,1,1,1,1])
	bilateralCoords =tf.reshape(tf.concat([coords/alpha,I/beta],4),[batch_size,-1,int(I.get_shape()[-1])+3])
	spatialCoords=tf.reshape(coords/gamma,[batch_size,-1,3])
	kernel_coords=[bilateralCoords,spatialCoords]
	permutohedrals = [permutohedral_prepare(coords) for coords in kernel_coords]
	with tf.variable_scope(name):
		nCh=U.get_shape()[-1]
		mu = tf.Variable(numpy.reshape(numpy.eye(nCh),[1,1,1,nCh,nCh]),dtype=tf.float32,name='Compatibility');
	...
	for t in range(T):
		H1.append(ftheta(U,H1[-1],permutohedrals,mu,kernel_weights, aspect_ratio=aspect_ratio,name=name+str(t)))
	return H1
