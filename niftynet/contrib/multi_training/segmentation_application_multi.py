...
class SegmentationApplicationMulti(BaseApplication):
    REQUIRED_CONFIG_SECTION = ["SEGMENTATION"]


    def __init__(self, net_param, action_param, action):
        super(SegmentationApplicationMulti, self).__init__()
    ...
    def initialise_network(self):
        w_regularizer = None
        b_regularizer = None
        self.training_mode = self.action_param.training_mode_start
        self.training_types = self.action_param.training_types
        self.training_values = self.action_param.training_values
    ...
    def connect_data_and_network(self,
        ...
        if self.is_training:
            ...
            def loss_2():
                return loss * loss + 10*loss
            self.total_loss = tf.cond(tf.equal(self.training_mode, 1), loss_2,
                                      loss_1)
            var_train = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES)
            var_output = [var for var in var_train if 'output' in var.name]
            grads = self.optimiser.compute_gradients(
                loss, colocate_gradients_with_ops=True)
            grads2 = self.optimiser.compute_gradients(loss*loss + 10*loss,
                                                      colocate_gradients_with_ops=True, var_list=var_output)
            grads3 = self.optimiser.compute_gradients(loss * loss + 10 * loss,
                                                      colocate_gradients_with_ops=True,
                                                      var_list=var_train)
            # collecting gradients variables
            gradients_collector.add_to_collection([grads, grads2, grads3])
