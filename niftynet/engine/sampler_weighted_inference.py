...
class WeightedSamplerInference(UniformSampler):
    """
    ...
    def __init__(self,
                 ...
                 windows_per_image=1,
                 queue_length=10,
                 name='weighted_sampler_inference'):
...
def weighted_spatial_coordinates(
        ...
        min_intensity=0.1, max_intensity=1):

    #img_spatial_size = tf.Print(img_spatial_size, [tf.shape(img_spatial_size)],
    #                            message="Image size")
    """
    ...
    """
    tf.logging.info(
        'Sampling from {} and {} intensity range'.format(min_intensity,
                                                         max_intensity))
    ...
    for sample in range(0, n_samples):
        ...
        # sample from a specified intensity range
        # so we can set a treshold as well
        step = ((max_intensity-min_intensity)/(n_samples+1))
        sampling_ratios = np.arange(min_intensity,max_intensity,step)
        sample_ratio = sampling_ratios[sample]
