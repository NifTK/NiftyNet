    ...
    def layer_op(self, random_source, image_size, conditioning, is_training):
        ...
        def conv(ch, x):
            with tf.name_scope('conv'):
                conv_layer = ConvolutionalLayer(ch, 3, featnorm_type=None, w_initializer=w_init)
                c = conv_layer(x, is_training=is_training)
                return tf.nn.relu(tf.contrib.layers.batch_norm(c))

        def up(ch, x, hack=False):
            with tf.name_scope('up'):
                deconv = DeconvolutionalLayer(ch, 3, featnorm_type=None, stride=2, w_initializer=w_init)(x, is_training=is_training)
                if hack:
        ...
        with tf.name_scope('final_image'):
            ...
            x_sample = ConvolutionalLayer(1, 3, featnorm_type=None, with_bias=True,
    ...
    def layer_op(self, image, conditioning, is_training):
        ...
        def down(ch, x):
            with tf.name_scope('downsample'):
                c = ConvolutionalLayer(ch, 3, stride=2, featnorm_type=None,
        ...
        def convr(ch, x):
            c= ConvolutionalLayer(ch, 3, featnorm_type=None,
                                  w_initializer=w_init)(x, is_training=is_training)
