...
data_param = {'input': ParserNamespace(pixdim=(),
              ...
              'output': ParserNamespace(pixdim=(),
                                        ...
                                        interp_order=0)}
set_number_of_memory_subjects(data_param, NUM_SUBJECTS)
...
class MemoryImageSetsPartitionerTest(tf.test.TestCase):
    def test_file_split_file_error(self):
        ...
        test_partitioner = MemoryImageSetsPartitioner()
    ...
    def test_partitioning(self):
        ...
        for val_frac in (0.0, 0.2, 0.4):
            ...
            all_idcs = test_partitioner.get_image_lists_by(phase=ALL)
            ...
            train_idcs = test_partitioner.get_image_lists_by(phase=TRAIN)[0]
            self.assertTrue(abs(len(train_idcs) - ref_train) <= 1)
            val_idcs = test_partitioner.get_image_lists_by(phase=VALID)[0]
            self.assertTrue(abs(len(val_idcs) - ref_val) <= 1
                            and not any(idx in train_idcs for idx in val_idcs))
            infer_idcs = test_partitioner.get_image_lists_by(phase=INFER)[0]
