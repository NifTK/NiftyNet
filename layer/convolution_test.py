...
class ConvTest(tf.test.TestCase):
    def test_shape(self):
        ...
        conv_3d = ConvLayer(tf.nn.conv3d,
                10, [3, 3, 3], [1, 1, 1, 1, 1], with_bn=True)
        input_shape = (2, 16, 16, 16, 8)
        x = tf.ones(input_shape)
        conv_3d_out = conv_3d(x, is_training=True)
        print conv_3d

        conv_2d = ConvLayer(tf.nn.conv2d,
                10, [3, 3], [1, 1, 1, 1], with_bn=True)
        input_shape = (2, 16, 16, 8)
        x = tf.ones(input_shape)
        conv_2d_out = conv_2d(x, is_training=False)
        print conv_2d
        #bn_layer = BNLayer()
