...
def run(net_class, param, volume_loader, device_str):
    ...
    with graph.as_default(), tf.device("/{}:0".format(device_str)):
        ...
        output = net(test_pairs['images'], is_training=False)

        # converting predictions into final output for
        ...
        net_out = post_process_layer(output)
        ...
        try:
            ...
            if param.vae_inference_application == 'forward_pass':
                ...
                while True:
                    ...
                    current_output = current_output[2]  # These are the data means
                    ...
                    for batch_id in range(current_output.shape[0]):
                        ...
                        predictions = np.expand_dims(current_output[batch_id], axis = -1)  # ...NII requirements...
                ...
                dictionary_tmp = {net_out[-1]: noise}.copy()
                current_decoded_interpolated_code = net_out[2].eval(feed_dict=dictionary_tmp)
                ...
                # Linearly interpolate between the encodings of the first two volumes in the queue,
                ...
                dictionary_tmp = {net_out[-1]: lin_interp_codes}.copy()
                current_decoded_interpolated_code = net_out[2].eval(feed_dict=dictionary_tmp)
