...
class VolumeSampler(object):
    def __init__(self, f_names,
                 ...
                 volume_padding, sample_per_volume=0,dict_preprocess={'rotation':1,'normalisation':1,'spatial_scaling':1},
...
# Function to make the dict_sampling consistent if some fields are missing
    def adapt_dict_sampling(self):
        print('adaptation of dict_sampling')
        ...
        if 'flag_pad_effect' not in self.dict_sampling.keys():
            self.dict_sampling['flag_pad_effect'] = 0
        return
    ...
    def strategic_sampling(self,img,seg):
        ...
        while flag_test==0 and iter<200:
            ...
            if self.dict_sampling['flag_pad_effect']:
                seg_test = seg[xs+self.padding:xe-self.padding, ys+self.padding:ye-self.padding, zs+self.padding:ze-self.padding]
            else:
            ...
            if len(inter) < Lcomp_values:
                ...
                if len(self.dict_sampling['minimum_sampling_elements'])!=0:
                    if len(self.dict_sampling['minimum_sampling_elements'])>=len(indices) and len(list(set(indices) - set([-1])))>0:
                        min_sample = [self.dict_sampling['minimum_sampling_elements'][i] for i in range(len(indices))]
                    else :
                    ...
                    for i in range(0,len(inter)):
                        if np.count_nonzero(seg_test == inter[i])<min_sample[i]:
                            ...
                            break
                if len(self.dict_sampling['minimum_sampling_elements'])!=0:
                    if len(self.dict_sampling['minimum_ratio'])>=len(indices):
                        min_ratio = [self.dict_sampling['minimum_ratio'][i] for i in range(len(indices))]
                    else:
